<?php
require('header.php');
?>

<script>
  document.getElementById("headerImage").style.backgroundImage = "url('img/headerImages/rusticWood.jpg')";
  document.getElementById("headerImage").style.backgroundPosition = "50% 35%";
</script>

  <!--
    General layout inherited from extras.html
  -->
  
  
  <div id="buttonDiv" class="row justify-content-center">
  <button onclick="window.location.href='https://orange.greenriverdev.com/weddingRentals/extras.php';" id="checkAvailabilityButton" type="submit" class="btn">Extras Page</button></button>
</div>
  <section class= "sectionPick6set">
   <h4 class="section-head sectionSetDescription">Rustic Wood Full Package Rental</h4
  <div class="container">
  <div class="row">
    <div class="col-sm">
      $299 plus Tax
      <ul class="list-group">
          <li>“Welcome to Our Beginning” (approximately 32” x 18 ”with easel) Your choice of white framed, or rustic stained</li>
          <li>“Find your Seat” (32.5” x 18.5” framed organizer with 25 clips & easel)</li>
          <li>Table Numbers (1-25 3.5”x 3.5” with mini easels) (50 pc)</li>
          <li>Antique Glass Jug with “Honeymoon Fund” (jug & mini- hanger 4.75”x 10”) (2pc)</li>
          <li>“Mr. & Mrs.” Head Table Blocks (3pc)</li>
          <li>“We know that you would be here today if Heaven weren’t so far away” (9.75” x 16” framed memorial seat saver)</li>
          <li>Here Comes Your Bride” ring bearer carrier (9.75 x 15” framed with cord)</li>
          <li>“Mr. & Mrs.” Chair Hangers (with cord 9.75” x 15”) (2pc)</li>
          <li>“Guestbook please sign” (self standing 5.5” x 20”)</li>
          <li>“Just Married” & “Thank You” (Reversible photo-shoot prop 5.5” x 29.5”)</li>
          <li>"Take One” (5.5” x 6”)</li>
          <li>“Programs” (15.75” x 5.5”)</li>
          <li>“In Loving Memory of those who are forever in our hearts” (6”x 5.5” self standing)</li>
          <li>8 “Reserved” signs (2.75” x 10” 4 with cord hanger option) (8pc)</li>
          <li>Antique Whitewashed Trunk with “Cards” Banner</li>
      </ul>
    </div>
    <div class="col-sm">
      <img src="img/rusticWoodSetImages/rusticWoodSetWelcome.jpg" class="img-fluid" alt="Responsive image">
    </div>
  </div>
</div>
</section>

  <hr>



<section class= "sectionPick6set">
   <h4 class="section-head sectionSetDescription">Rustic Wood “No Seating” Rental</h4
  <div class="container">
  <div class="row">
    <div class="col-sm">
      $245 plus Tax
      <ul class="list-group">
          <li>“Welcome to Our Beginning” (approximately 32” x 18 ”with easel) Your choice of white framed, or rustic stained</li>
          <li>Antique Glass Jug with “Honeymoon Fund” (jug & mini- hanger 4.75”x 10”) (2pc)</li>
          <li>“Mr. & Mrs.” Head Table Blocks (3pc)</li>
          <li>“We know that you would be here today if Heaven weren’t so far away” (9.75” x 16” framed memorial seat saver)</li>
          <li>Here Comes Your Bride” ring bearer carrier (9.75 x 15” framed with cord)</li>
          <li>“Mr. & Mrs.” Chair Hangers (with cord 9.75” x 15”) (2pc</li>
          <li>“Guestbook please sign” (self standing 5.5” x 20”)</li>
          <li>“Mr. & Mrs.” Chair Hangers (with cord 9.75” x 15”) (2pc)</li>
          <li>“Guestbook please sign” (self standing 5.5” x 20”)</li>
          <li>“Just Married” & “Thank You” (Reversible photo-shoot prop 5.5” x 29.5”)</li>
          <li>"Take One” (5.5” x 6”)</li>
          <li>“Programs” (15.75” x 5.5”)</li>
          <li>“In Loving Memory of those who are forever in our hearts” (6”x 5.5” self standing)</li>
          <li>8 “Reserved” signs (2.75” x 10” 4 with cord hanger option) (8pc)</li>
          <li>Antique Whitewashed Trunk with “Cards” Banner</li>
      </ul>
    </div>
    <div class="col-sm">
      <img src="img/rusticWoodSetImages/rustedWoodSetJustMarried.jpg" class="img-fluid" alt="Responsive image">
    </div>
  </div>
</div>
</section>

  <hr>

  <div class="row">
    <div class="col">
         <section class= "sectionPick6set">
   <h4 class="section-head sectionSetDescription">Rustic Wood “You Pick Four” Rental</h4>
<p id="pick4directions">SELECT ANY 4 OF THE FOLLOWING</p>
   <p id="pick4price">$199 Plus Tax</p>
</section>
    </div>
</div>

   <div class="row">
  <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image1.jpg" alt="Card image cap">
    </div>
  </div>
  <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image2.jpg" alt="Card image cap">
    </div>
  </div>
     <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image3.jpg" alt="Card image cap">
    </div>
  </div>
     <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image4.jpg" alt="Card image cap">
    </div>
  </div>
     <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image5.jpg" alt="Card image cap">
    </div>
  </div>
     <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image6.jpg" alt="Card image cap">
    </div>
  </div>
     <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image7.jpg" alt="Card image cap">
    </div>
  </div>
     <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image8.jpg" alt="Card image cap">
    </div>
  </div>
     <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image9.jpg" alt="Card image cap">
    </div>
  </div>
     <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image10.jpg" alt="Card image cap">
    </div>
  </div>
     <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image11.jpg" alt="Card image cap">
    </div>
  </div>
     <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image12.jpg" alt="Card image cap">
    </div>
  </div>
     <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image13.jpg" alt="Card image cap">
    </div>
  </div>
     <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image14.jpg" alt="Card image cap">
    </div>
  </div>
     <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image15.jpg" alt="Card image cap">
    </div>
  </div>
     <div class="col-sm-6" style="width: 18rem; padding:2%; ">
    <div class="card">
        <img class="card-img-top" src="img/rusticWoodSetImages/image16.jpg" alt="Card image cap">
    </div>
  </div>
</div>
  
  

 

  
  



<div class="container">
  <div class="row">
    <div class="col-sm">
   
     <ul class="list-group youPickFourList">
          <li>“Welcome to Our Beginning” (approximately 32” x 18 ”with easel) Your choice of white framed, or rustic stained</li>
          <li>“Find your Seat” (32.5” x 18.5” framed organizer with 25 clips & easel)</li>
          <li>Table Numbers (1-25 3.5”x 3.5” with mini easels) (50 pc)</li>
          <li>Antique Glass Jug with “Honeymoon Fund” (jug & mini- hanger 4.75”x 10”) (2pc)</li>
          <li>“Mr. & Mrs.” Head Table Blocks (3pc)</li>
          <li>“We know that you would be here today if Heaven weren’t so far away” (9.75” x 16” framed memorial seat saver)</li>
          <li>Here Comes Your Bride” ring bearer carrier (9.75 x 15” framed with cord)</li>
          <li>“Mr. & Mrs.” Chair Hangers (with cord 9.75” x 15”) (2pc)</li>
          <li>“Guestbook please sign” (self standing 5.5” x 20”)</li>
          <li>“Just Married” & “Thank You” (Reversible photo-shoot prop 5.5” x 29.5”)</li>
          <li>"Take One” (5.5” x 6”)</li>
          <li>“Programs” (15.75” x 5.5”)</li>
          <li>“In Loving Memory of those who are forever in our hearts” (6”x 5.5” self standing)</li>
          <li>8 “Reserved” signs (2.75” x 10” 4 with cord hanger option) (8pc)</li>
          <li>Antique Whitewashed Trunk with “Cards” Banner</li>
      </ul>
    </div>
    <div class="col-sm">
   
    </div>
  </div>
</div>




 

  <hr>


<div class="row">
    <div class="col">
         <section class= "sectionPick6set">
   <h4 class="section-head sectionSetDescription">Choose either “Welcome Sign” below when selecting your rental package…</h4>
</section>
    </div>
</div>

  <img src="img/rusticWoodSetImages/welcome1.jpg">



  <section class= "sectionPick6set">
  <div class="container">
  <div class="row">
    <div id="welcomeSignDiv" class="col-sm">
      <h1>WELCOME SIGNS INCLUDE EASEL FOR DISPLAY</h1>
      <p>Choose from “Stained Wood” (above) </p>
      <p>or</p>
      <p>“Framed White” (right)</p>
    </div>
    <div class="col-sm">
      <img src="img/rusticWoodSetImages/welcome2.jpg" class="img-fluid" alt="Responsive image">
    </div>
  </div>
</div>
</section>
  

<hr>


  <section class= "sectionPick6set">
  <div class="container">
  <div class="row">
    <div class="col-sm">
      <img src="img/rusticWoodSetImages/welcome3.jpg" class="img-fluid" alt="Responsive image">
    </div>
    <div id="welcomeSignDiv" class="col-sm">
      <h1>AISLE RUNNER ADD-ON</h1>
      <p>Six signs featuring phrases of the 1 Corinthians 13 “Love Chapter.”</p>
      <p>$99 with any rental package.</p>
    </div>
  </div>
</div>
</section>

  <hr>


  <section class= "sectionPick6set">
  <div class="container">
  <div class="row">
    <div id="welcomeSignDiv" class="col-sm">
      <h1>VINTAGE TYPE-WRITERWITH MESSAGE TO GUESTS</h1>
      <p>Travels in custom vintage</p>
      <p>Walnut Ridge shipping crate.</p>
      <p>$99 with any rustic wood rental package.</p>
    </div>
    <div class="col-sm">
      <img src="img/rusticWoodSetImages/welcome4.jpg" class="img-fluid" alt="Responsive image">
    </div>
    
  </div>
</div>
</section>

  <hr>

  <p id="photoCred">Product images courtesy of ROSIE photography.</p>

  

  <div id="buttonDiv" class="row justify-content-center">
  <button onclick="window.location.href='https://orange.greenriverdev.com/weddingRentals/extras.php';" id="checkAvailabilityButton" type="submit" class="btn">Extras Page</button></button>
</div>

<?php
//footer
require('footer.php');
?>
